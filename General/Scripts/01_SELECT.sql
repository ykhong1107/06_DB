...
-- EMPLOYEE 테이블에서
-- 모든행의 이름(EMP_NAME), 급여(SALARY) 컬럼조회
SELECT EMP_NAME, SALARY 
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서
-- 모든행의 사번, 이름, 급여 입사일 조회

SELECT EMP_ID, EMP_NAME, SALARY, HIRE_DATE 
FROM EMPLOYEE;

-- EMPLOYEE 테이블에 존재하는 모든행, 모든 컬럼조회
...
SELECT EMP_ID AS 사번, -- 컬럼명 AS 별칭
			 EMP_NAME 이름,  -- 컬럼명 별칭(AS생략)
			 HIRE_DATE AS "입사한 날짜",-- 컬럼명 AS "별칭"
	 		 CURRENT_DATE - HIRE_DATE "@근무 일수@", 
	     CEIL((CURRENT_DATE - HIRE_DATE) / 365) "N연째 근무중..."
FROM EMPLOYEE;
...
/* SELECT절에 컬럼명이 아닌 리터럴(값) 작성하는 경수 
 * 
 * - 조회결과에 작성된 리터럴 컬럼이 추가되고,
 *   모든행에 리터럴이 작성되어 있음
 * */
...
WHERE PHONE LIKE '010________'; -- _ 8개
 ...
 * 		컬럼명 | 별칭 | 컬럼순서(숫자)
 *    [ASC / DESC]  (오름차순/내림차순)
 * 		[NULLS FIRST / NULLS LAST] (NULL 위치지정)
 * 
...
--> WHERE 절은 SELECT보다 먼저해석되는데
-- 별칭은 SELECT절이 해석되어야 등록되기 때문에
-- WHERE절에서는 별칭사용이 불가능하다!!!!!!!!!!!!!!! 
-- ★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆	 
...
/* NULLS FIRST / NULLS LAST(기본 값) 확인 */

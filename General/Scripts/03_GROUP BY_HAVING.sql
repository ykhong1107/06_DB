...
-- 부서코드 순으로 정렬
SELECT							-- 4. 조회 결과에 포함될 컬럼지정
	DEPT_CODE,
	COUNT(*)
FROM								-- 1. 어떤 테이블에서
	EMPLOYEE					
WHERE							  -- 2. 어떤 행만 조회할지 결정
	BONUS IS NOT NULL
GROUP BY						-- 3. WHERE 결과에서 그룹 구분
	DEPT_CODE
ORDER BY						-- 5. 조회결과 정렬
	DEPT_CODE ASC;
...
-- 부서별 평균급여 3500000원 이상인 부서를 조회하여 부서코드 오름차순으로 정렬
SELECT
	DEPT_CODE,
	AVG(SALARY)
FROM 
...
--> 그룹별 급여평균이 350만 이상인 그룹만 조회(맞는 SQL)
...
GROUP BY
	JOB_CODE
ORDER BY
	JOB_CODE ASC; -- 4행 조회(0명은 조회결과헤 포함 X)
	
-- 2-1.EMPLOYEE 테이블에서 각 직급별 보너스 유무포함 사원의 수를 조회하여
-- 직급코드 오름차순으로 정렬하세요

/* 통계적인 측면에서는 아래방법이 더 좋다!! */
--> COUNT(컬럼명) : 해당 컬럼에 값에 행이 몇개인지 카운트
----> NULL이면 카운트에서 제외	
SELECT
	JOB_CODE,
	COUNT(BONUS)
...
INTERSECT
-- 급여가 400만을 초과하는 사원의 이름, 부서코드, 급여조회
SELECT
	EMP_NAME,
	DEPT_CODE,
	SALARY

...
/* 관리자 계정*/
/* TEST_USER 계정에 접속 권한추가 */

-- 권한 부여방법 : GRANT 권한, 권한 .... TO 계정명;
GRANT CREATE SESSION TO TEST_USER;

/* TEST_USER 계정에는
 * 데이터베이스 접속권한인 CREATE SESSION만 부여됨.
 * 
...
-- RESOURCE ROLE : 8개 객체제어 권한을 묶어둔 ROLE
/*CREATE SEQUENCE
CREATE PROCEDURE
CREATE CLUSTER
CREATE INDEXTYPE
...
-- SELECT 권한을 부여받은 KH계정의 EMPLOYEE 테이블 조회
SELECT * FROM KH_HYK.EMPLOYEE;

-- 다른 테이블 조회도 되는지 확인
SELECT * FROM KH_HYK.DEPARTMENT;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
   --> 조회권한이 없어서 테이블 존재여부초차 모르기 때문에 발생하는 에러
...
SELECT * FROM KH_HYK.EMPLOYEE; -- 조회불가
...
-- TEST_USER 계정삭제(DROP USER 계정명)
DROP USER TEST_USER;
-- ORA-01940: 현재 접속되어있는 사용자는 사용할 수 없습니다
 --> Database Navigator 에서
  --> TEST_USER 계정에 접속중이라서 삭제불가
   --> 연결종료

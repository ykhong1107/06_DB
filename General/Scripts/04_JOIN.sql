...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
		...
		--> DEPT_CODE와 DEPT_ID값이 같이 행을 한 행으로 합쳐라
...
--> DEPT_CODE, DEPT_ID가 일치하는 행끼리 한행으로 합쳐짐
...
-- ANSI 방식
SELECT DEPT_TITLE 부서명, LOCAL_NAME 지영멱
FROM DEPARTMENT
/*INNER*/ JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);

--> INNER JOIN이 JOIN의 기본 값(생략가능)
...
--> JOB_CODE 컬럼명이 같아서 어떤 테이블의 컬럼인지 구분할 수 없기 때문에 오류남

--> 해결방법 : JOB_CODE 앞에 테이블명 또는 별칭작성
...
--> 직접사용보단 NATURAL JOIN의 실패로 보여지는 모습이다
--  라고 알아두는게 좋음
...
-- 사원의 급여가
-- SAL-LEVEL 에 작성된 최소(MIN_SAL) ~ 최대(MAX_SAL)
...
-- EMPLOYEE 테이블에서
-- 모든사원의 이름과, 사수번호(MANAGER_ID), 사수이름 조회
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) -- 예외처리를 위해 LEFT
...
CREATE SEQUENCE SEQ_USER_NO NOCACHE; -- 1부터 시작해서 1개씩 늘어날예정

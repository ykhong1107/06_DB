...
HAVING SUM(SALARY) = 
	(SELECT MAX(SUM(SALARY))
	FROM EMPLOYEE
	GROUP BY DEPT_CODE); -- 서브쿼리 결과가 2176000
...
HAVING COUNT(*) = 
		(SELECT MIN(COUNT(*))
		FROM EMPLOYEE
		GROUP BY DEPT_CODE); -- 서브쿼리결과 : 2
		
/***** 서브쿼리에서 사용한 별칭을 메인 쿼리에서 사용하기 *****/
-- 인라인뷰 : FROM절에 사용된 서브쿼리
		--> 서브쿼리 결과가 테이블처럼 인식이된다.
...
SELECT DISTINCT MANAGER_ID       -- DISTINCT : 중복제거(별개의//)
FROM EMPLOYEE
...
-- 방법 1) UNION을 이용하는 방법
	 --> UNION : 두 SELECT의 결과(RESULT STE)을 하나로 합침(합집합)
SELECT 
	EMP_ID, 
	EMP_NAME, 
	NVL(DEPT_TITLE, '없음') AS DEPT_TITLE, 
...
--> 여러컬럼을 묶어서 한번에 비교해
--  모두 같은행만 조회결과(RESULT SET)에 포함시킴
...
WHERE EXISTS( -- 서브쿼리 조회결과가 있으면 TRUE == 해당행을 결과에 포함하겠다
	SELECT '있음' -- 서브쿼리에서 조회되는 컬럼값은 중요한게 아님!!
...
-- 3) 각 직원의 이름, 직급코드, 직급코드, "직급별 급여평균" 조회
SELECT 	
	EMP_NAME, 
	JOB_CODE,
	(SELECT AVG(SALARY)
...
-- 7. 인라인 뷰(INLINE-VIEW) --> SELECT문에서 조회되는 가상테이블

/* VIEW(객체) : 조회용도의 가상테이블
 * -> SELECT는 가능핮미나 INSERT, UPDATE, DELETE는 불가함
 * */
...
FROM
	(SELECT EMP_NAME, SALARY
	 FROM EMPLOYEE
	 ORDER BY SALARY DESC)
	 --> 서브쿼리 결과(23행 2열)을 메인쿼리의 테이블로 인식
WHERE
	ROWNUM <= 5;
...
FROM
	(SELECT 
		...
		ORDER BY "급여 평균" DESC)
WHERE ROWNUM <= 3; -- 서브쿼리 조회결과에서 1,2,3 행만 조회
...
-- 1) RANK() OVER : 
--		OVER() 괄호에 작성된 정렬 기준대로 정렬 후 순위지정
--    단, 값의 크기가 같다면 공동 순위지정, 지정된 만큼 순위 건너뛰기
...
--> ROWNUM은 RESULT SET 완성 후 적용되는 가상컬럼이라서
...
FROM
		...
		FROM EMPLOYEE)
WHERE
	  ROWNUM BETWEEN 3 AND 7; -- 3행이상 7행이하
-- 	ROWNUM BETWEEN 1 AND 7; -- 3행이상 7행이하
...
-- 2) 1번결과를 다중행 서브쿼리로 사용해서 생일이 일치하는 사원들 조회
SELECT 
	EMP_ID,
	EMP_NAME,
	JOB_NAME,

...
-- 2000년도 이전 학번을 받은 학생들의 학번과 이름을 조회하는 SQL을 작성하시오.
SELECT
	STUDENT_NO,
	STUDENT_NAME
FROM
...
-- 7번 학번이 A517178인 한아름 학생의 학점 총 평점을 구하는 SQL문을 작성하시오. 
...
SELECT
	DEPARTMENT_NO 학과코드명,
	COUNT(DECODE(ABSENCE_YN,'Y','Y','NO',0))휴학생수	
FROM
	TB_STUDENT
GROUP BY
	DEPARTMENT_NO
...
SELECT
	DEPARTMENT_NO 학과코드명,
	DECODE(ABSENCE_YN,'Y','Y','NO',0) 휴학생수	
FROM
	TB_STUDENT
...
-- 단, 출력헤더에는 "학생이름", "학번", "거주지 주소"가 출력되도록
SELECT
	STUDENT_NAME 학생이름,
	SUBSTR(STUDENT_SSN, 1, 6) 학번,

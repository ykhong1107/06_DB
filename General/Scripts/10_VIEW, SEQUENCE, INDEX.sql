...
--> 계정명이 언급되는 상황에서는 위에구문 반드시 실행!!

-- VIEW 생성 권한부여
GRANT CREATE VIEW TO KH_HYK;
...
-- 사번, 이름, 부서명, 직급명을 쉽게 조회하기위한 VIEW 생성
...
-- VIEW에 INSERT를 수행했지만
-- VIEW를 만들때 사용한 원본테이블에
-- 값이 INSERT 됨을 확인!!!

 --> 하지만 모든 컬럼값이 INSERT 된 것이 아니라
 --  VIRE를 생성할 때 사용된 컬럼에만 값이 INSERT되어
 --  반대로 사용되지 않은 컬럼에는 NULL이 들어감!!
	 --> NULL은 DB의 무결성을 약하게 만드는 주요원인!!
--  	(가능하면 의도되지 않은 NULL은 조재하지 않게하자!!)
...
--  가장 최근 호출된 NEXTVAL의 값을 반환함을 뜻함
 --> NEXTVAL을 호출한적이 없어서 오류발생!!

--> 해결방법 : NEXTVAL을 호출하면 생김
...
-- '짱구'의 PK 컬럼값을 
-- SEQ_TEXT_NO 시퀀스의 다음생성값으로 변경하기
UPDATE TB_TEST
SET
 	TEST_NO = SEQ_TEST_NO.NEXTVAL
WHERE TEST_NAME = '짱구';
--> 짱구의 TEST_NO 값을 150까지 증가시키고 나서
 --> 다시한번더 SEQ_TEST_NO.NEXTVAL 호출
